...
namespace {
    ...
    /**
     * @tc.name: BenchmarkTestForIsValid
     * @tc.desc: Testcase for testing 'IsValid' function.
     * @tc.type: FUNC
     * @tc.require: Issue Number
     */
    static void BenchmarkTestForIsValid(benchmark::State &state)
    {
        FormInfo info;
        info.package = "com.ohos.contactsdataability";
        info.bundleName = "com.ohos.contactsdataability";
        ...
        info.description = "dataability_description";
        for (auto _ : state) {
            /* @tc.steps: step1.call IsValid in loop */
            info.IsValid();
        }
    }
    ...
    BENCHMARK(BenchmarkTestForIsValid)->Iterations(1000);
}
